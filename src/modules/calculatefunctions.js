...
export function dobToWords(dob) {
  const months = [
    "January",
    "February",
    "March",
  ...
  const dayWord = ordinals[day];
  const monthWord = months[month - 1];

  // break year into thousands/hundreds
  const yearWord = numberToWords(year).replace(/\b\w/g, (c) => c.toUpperCase()); // capitalize first letters
